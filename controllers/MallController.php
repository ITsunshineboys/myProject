...
//        $pid == 0 && array_unshift($categories, GoodsCategory::forAll());
    ...
    public function actionCategoryGoods()
    {
        ...
        $brandId = trim(Yii::$app->request->get('brand_id'), ', ');
        $styleId = trim(Yii::$app->request->get('style_id'), ', ');
        $seriesId = trim(Yii::$app->request->get('series_id'), ', ');
        ...
        $brandId && $where .= " and brand_id in({$brandId})";
        $styleId && $where .= " and style_id in ({$styleId})";
        $seriesId && $where .= " and series_id in ({$seriesId})";
        ...
        return Json::encode([
            ...
            'data' => [
                'brand_application_review_list' => [
                    ...
                    'applied_cnt' => (int)BrandApplication::find()->where(['review_status' => Yii::$app->params['reviewStatuses'][0]])->asArray()->count(),
                ]
            ],
        ]);
    }
...
//        $cache->set($cacheKey, $data, Yii::$app->params['goods']['viewCacheTime']);
    ...
    public function actionCheckRoleGetIdentity()
    {
        ...
        is_int($checkRoleRes) && $checkRoleRes = new User;
        return Json::encode([
            'code' => 200,
            'msg' => 'OK',
            'data' => [
