;
let waitpay_detail = angular.module("waitsend_detail_module", ['ui.bootstrap']);
waitpay_detail.controller("waitsend_detail_ctrl", function ($scope, $http, $stateParams,$state,$uibModal) {
  let config = {
  ...
  // 同意按钮
    ...
    console.log($scope.sku)
        $http.post(baseUrl+'/order/refund-handle',{
            order_no:$scope.order_no,
            sku:$scope.sku,
            handle:1
        },config).then(function (res) {
   ...
   //修改快递单号确认按钮
    ...
    $scope.change_track_btn=function () {
        if(!!$scope.track_input_model){
            $http.post(baseUrl+'/order/expressupdate',{
                order_no:$scope.order_no,
                sku:$scope.sku,
                ...
                if(res.data.code==200){
                    ...
                    $scope.shipping_way=res.data.data.shipping_way;
                }else{
