...
class Goods extends ActiveRecord
{
    ...
    public static function priceDetail($level, $title, $city = self::DEFAULT_CITY)
    {
        ...
        //TODO 修改
        if(is_array($title)){
            $where=['and', ['logistics_district.district_code' => $city], ['gc.level' => $level], ['in', 'gc.id', $title],['in','goods.status', self::STATUS_ONLINE]];
        }else{
            $where=['and', ['logistics_district.district_code' => $city], ['gc.level' => $level], ['gc.id'=>$title], ['in','goods.status', self::STATUS_ONLINE]];
        }
