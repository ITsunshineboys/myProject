...
AND goods.id IN (" . $id . ")";
    ...
    public static function categoryById($all = [], $city = 510100)
    {
        if ($all) {
            ...
            $id = implode('\',\'', $material);
            $db = Yii::$app->db;
            $sql = "SELECT goods.id,goods.platform_price,goods.supplier_price,goods.purchase_price_decoration_company,goods_brand.name,gc.title,logistics_district.district_name,goods.series_id,goods.style_id,goods.subtitle,goods.profit_rate,gc.path FROM goods LEFT JOIN goods_brand ON goods.brand_id = goods_brand.id LEFT JOIN goods_category as gc ON goods.category_id = gc.id LEFT JOIN logistics_template ON goods.supplier_id = logistics_template.supplier_id LEFT JOIN logistics_district ON logistics_template.id = logistics_district.template_id WHERE logistics_district.district_code =" . $city . " AND gc.title IN ('" . $id . "') AND goods.profit_rate = (SELECT MAX(goods.profit_rate) FROM goods LEFT JOIN goods_brand ON goods.brand_id = goods_brand.id LEFT JOIN goods_category ON goods.category_id = goods_category.id LEFT JOIN logistics_template ON goods.supplier_id = logistics_template.supplier_id LEFT JOIN logistics_district ON logistics_template.id = logistics_district.template_id WHERE goods_category.title = gc.title)";
            $all_goods = $db->createCommand($sql)->queryAll();
            return $all_goods;
        }
    }
