...
class Goods extends ActiveRecord
{
    ...
    public static function disableGoodsByCategoryId($categoryId, ActiveRecord $operator, $offlineReason = '')
    {
        ...
        $where = ['status' => self::STATUS_WAIT_ONLINE, 'category_id' => $categoryId];
        self::updateAll([
            'reason' => $offlineReason ? $offlineReason : Yii::$app->params['category']['offline_reason'],
        ], $where);
    }
    ...
    public static function disableGoodsByCategoryIdsBrandIds(array $categoryIds, $brandId, ActiveRecord $operator, $offlineReason = '')
    {
        ...
        self::updateAll([
            'status' => self::STATUS_OFFLINE,
            'offline_time' => time(),
            'offline_reason' => $offlineReason ? $offlineReason : Yii::$app->params['brandCategory']['offline_reason'],
            'offline_uid' => $operator->id,
            'offline_person' => $operator->nickname,
        ], $where);
    ...
    /**
     * Disable goods by style id
     *
     * @param int $styleId style id
     * @param ActiveRecord $operator $operator
     */
    public static function disableGoodsByStyleId($styleId, ActiveRecord $operator)
    {
        $goodsIds = GoodsStyle::goodsIdsOneStyle($styleId);
        ...
        self::updateAll([
            'status' => self::STATUS_OFFLINE,
            'offline_time' => time(),
            'offline_reason' => Yii::$app->params['style_series']['offline_reason'],
            'offline_uid' => $operator->id,
            'offline_person' => $operator->nickname,
        ], $where);
        ...
        self::updateAll([
            'reason' => Yii::$app->params['style_series']['offline_reason'],
        ], $where);
    }

    /**
     * Disable goods by series id
     *
     * @param int $seriesId series id
     * @param ActiveRecord $operator $operator
     */
    public static function disableGoodsBySeriesId($seriesId, ActiveRecord $operator)
    {
        $where = ['status' => self::STATUS_ONLINE, 'series_id' => $seriesId];
        self::updateAll([
            'status' => self::STATUS_OFFLINE,
            'offline_time' => time(),
            'offline_reason' => Yii::$app->params['style_series']['offline_reason'],
        ...
        $where = ['status' => self::STATUS_WAIT_ONLINE, 'series_id' => $seriesId];
        self::updateAll([
            'reason' => Yii::$app->params['style_series']['offline_reason'],
        ], $where);
    }
    ...
    public static function disableGoodsByBrandId($brandId, ActiveRecord $operator)
    {
        ...
        $where = ['status' => self::STATUS_WAIT_ONLINE, 'brand_id' => $brandId];
        self::updateAll([
            'reason' => Yii::$app->params['brand']['offline_reason'],
        ], $where);
    }
    ...
    public static function assortList($all, $city = 510100)
    {
        ...
        $select = "goods.id,goods.category_id,goods.platform_price,goods.supplier_price,goods.market_price,goods.purchase_price_decoration_company,goods_brand.name,gc.title,logistics_district.district_name,goods.series_id,goods.style_id,goods.subtitle,goods.profit_rate,gc.path,goods.cover_image,supplier.shop_name,goods.title as goods_name";
        $all_goods = self::find()
            ->select($select)
            ->asArray()
            ->leftJoin('goods_brand', 'goods.brand_id = goods_brand.id')
            ...
            ->leftJoin('logistics_district', 'logistics_template.id = logistics_district.template_id')
            ->leftJoin('supplier', 'goods.supplier_id = supplier.id')
            ->where(['logistics_district.district_code' => $city])
            ->andWhere(['in','gc.id',$all])
            ->andWhere(['goods.status' => self::STATUS_ONLINE])
            ->all();

        foreach ($all_goods as &$one_goods) {
            $one_goods['platform_price'] =  $one_goods['platform_price'] / 100;
            $one_goods['supplier_price'] =  $one_goods['supplier_price'] / 100;
            $one_goods['market_price']   =  $one_goods['market_price'] / 100;
            $one_goods['purchase_price_decoration_company'] =  $one_goods['purchase_price_decoration_company'] / 100;
    ...
    public function rules()
    {
        return [
            ...
            [['category_id', 'brand_id', 'supplier_price', 'platform_price', 'market_price', 'purchase_price_decoration_company', 'purchase_price_manager', 'purchase_price_designer', 'left_number', 'logistics_template_id', 'style_id', 'series_id', 'favourable_comment_number'], 'number', 'min' => 0],
...
//            [['style_id'], 'validateStyleId', 'on' => [self::SCENARIO_ADD, self::SCENARIO_EDIT]],
    ...
    public function adminView()
    {
        ...
        return [
            ...
            'attrs' => GoodsAttr::frontDetailsByGoodsId($this->category_id),
