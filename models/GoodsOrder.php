...
//        $role_id=$user->last_role_id_app;
        ...
        $unshipped=(new Query())
            ...
            ->select('g.id')
            ->leftJoin(OrderGoods::tableName().' as o','g.order_no=o.order_no')
            ->where("g.pay_status=1 and o.order_status=0 and shipping_sattus=0 ")
            ->count();
        $unreceiveed=(new Query())
            ->from(GoodsOrder::tableName().' as g')
            ->select('g.id')
            ->leftJoin(OrderGoods::tableName().' as o','g.order_no=o.order_no')
            ->where("g.pay_status=1 and o.order_status=0 and shipping_sattus=1 ")
            ->count();
        $completed=(new Query())
            ->from(GoodsOrder::tableName().' as g')
            ->select('g.id')
            ->leftJoin(OrderGoods::tableName().' as o','g.order_no=o.order_no')
            ->where("g.pay_status=1 and o.order_status=1 and shipping_sattus=2 ")
            ->count();
        $canceled=(new Query())
            ->from(GoodsOrder::tableName().' as g')
            ->select('g.id')
