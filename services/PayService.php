...
class PayService
{
   ...
   /**
    ...
    public function GetOrderOpenid($orders)
    {
            ...
            //触发微信返回code码
            $baseUrl = urlencode(Yii::$app->request->hostInfo."/order/wx-line-pay");
            $url = $this->__CreateOauthUrlForCode($baseUrl);
            return $url;
    }
    ...
    { 
        $baseUrl = urlencode(Yii::$app->request->hostInfo."/order/find-open-id");
        $url = $this->__CreateOauthUrlForCode($baseUrl);
        header("Location:".$url);
    }
    ...
    public function GetJsApiParametersApp($UnifiedOrderResult)
    {
        ...
        $jsapi->SetAppPaySign($UnifiedOrderResult['sign']);
        $parameters = json_encode($jsapi->GetValues());
        return $parameters;
    }
